...
public class Geometry {
	...
	public Geometry(Matrix matrix, Graphics g, int height, int width, int m,
			int n) {
		this.matrix = matrix;
		this.g = g;
		this.height = height;
	...
	public void draw() {
		for (int j = 0; j < faces.length; j++) {// LOOP THROUGH ALL THE LINES IN
			...
			for (int i = 0; i < faces[j].length; i++) {
				matrix.transform(vertices[faces[j][i]], a); // TRANSFORM BOTH
															// ENDPOINTS OF LINE
				matrix.transform(vertices[faces[j][(i + 1) % faces[j].length]],
						b);
				viewport(a, pa);
				viewport(b, pb);
