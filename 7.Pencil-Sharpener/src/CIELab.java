...
public class CIELab extends ColorSpace {
	...
	public boolean jnd(float[] rgbvalue_1, float[] rgbvalue_2){
		
		float[] c1 = fromRGB(rgbvalue_1);
		float[] c2 = fromRGB(rgbvalue_2);
		
		double delta = Math.sqrt(Math.pow((c2[0]-c1[0]),2)+Math.pow((c2[1]-c1[1]),2)+Math.pow((c2[2]-c1[2]),2));
		if(delta > 2.3){
			return true;
		}
		return false;
